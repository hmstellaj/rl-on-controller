...
class Config:
    ...
    class Lidar:
        ...
        # LiDAR 데이터 폴더 경로
        LIDAR_FOLDER = r"C:\Users\MSI\Documents\Tank Challenge\lidar_data"
    ...
    class DWA:
        ...
        🔧 수정사항 (v3):
        1. MAX_SPEED: 0.15 (적당한 속도)
        2. MAX_ACCEL: 0.5 (충분한 가속 - Dynamic Window 확보)
        3. MAX_YAW_RATE: 60° (회전 강화)
        4. TO_GOAL_COST_GAIN: 0.15 (목표 지향 약간 증가)
        5. STEERING_PENALTY: 0.05 (회전 거의 무페널티)
        """
        
        # 속도 제한 (🔧 적당한 속도 + 충분한 가속)
        MAX_SPEED = 0.8           # 최대 속도 (적당히)
        MIN_SPEED = -0.10          # 최소 속도 (후진)
        MAX_YAW_RATE = 90.0 * math.pi / 180.0  # 최대 회전 속도 (60°/s)
        
        # 가속도 제한 (🔧 충분히 높게 - Dynamic Window 넓게)
        MAX_ACCEL = 0.5            # 최대 가속도 (넉넉하게)
        MAX_DELTA_YAW_RATE = 300.0 * math.pi / 180.0  # 회전 가속도 (90°/s²)
        
        # 탐색 해상도
        V_RESOLUTION = 0.05        # 속도 탐색 해상도 (0.01 → 0.02)
        YAW_RATE_RESOLUTION = 2.0 * math.pi / 180.0  # 회전 해상도 (5°)
        
        # 예측 파라미터
        DT = 0.1                   # 시뮬레이션 시간 간격 (초)
        PREDICT_TIME = 3.0         # 예측 시간 (초) - 약간 단축
        
        # 비용 함수 가중치 (🔧 균형 잡힌 설정)
        TO_GOAL_COST_GAIN = 0.8   # 목표 지향 비용 (약간 증가)
        ...
        COLLISION_DISTANCE = 3.0   # 충돌 거리 (이내면 경로 무효)
    ...
    class SEQ4:
        ...
        # SEQ 4 전용 LOOKAHEAD (🔧 증가: 더 멀리 보고 우회)
        LOOKAHEAD_DIST = 10.0  # 전방 주시 거리 (m) - 5 → 10
...
class FusionConfig:
    ...
    min_height_threshold: float = 8.0  # ✅ 추가: 높이 기반 필터링 임계값 (m)
...
class StateMachineConfig:
    ...
    # SCAN 모드 설정
    scan_turret_speed: float = 0.05    # 터렛 회전 속도 (가중치)
